#!/usr/bin/env python3
"""
Medium-scale evaluation on 400/100 split with rate limiting.

This script:
1. Mines cases from 400 messages (max 100 cases)
2. Runs quality evaluation with rate limiting to avoid API limits
3. Provides cost estimation before running

Usage:
  source .venv/bin/activate
  python test/run_eval_400_100.py
"""
...
def estimate_costs(n_messages: int, n_eval_scenarios: int) -> dict:
    ...
    # Mining phase
    n_chunks = 5  # Approximate for 400 messages
...
def main():
    ...
    print("=" * 70)
    print("EVALUATION: 400 messages / 100 max cases")
    print("=" * 70)
    
    # Cost estimation
    est = estimate_costs(n_messages=400, n_eval_scenarios=15)
    
    print("\n📊 COST ESTIMATION:")
    print(f"\n1. Mining Phase (400 messages -> structured cases):")
    print(f"   - Chunks to process: {est['mining']['chunks']}")
    print(f"   - Tokens: ~{est['mining']['tokens']:,}")
    print(f"   - Cost: ${est['mining']['cost_usd']}")
    
    print(f"\n2. Evaluation Phase (~12-15 scenarios):")
    ...
    # Set environment variables for 400/100 scale
    os.environ["REAL_LAST_N_MESSAGES"] = "400"
    os.environ["REAL_MAX_CASES"] = "100"
    os.environ["REAL_EVAL_N"] = "12"  # Will be more if we find more cases
    
    print("\n" + "=" * 70)
    print("PHASE 1: Mining cases from 400 messages")
    print("=" * 70)
    ...
    print("\n⏳ Starting case mining (this will take ~5-8 minutes)...")
