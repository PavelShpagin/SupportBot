...
НЕ витягуй ці кейси повторно! Пропускай треди які відповідають вже витягнутим кейсам.
"""

P_CASE_SYSTEM = """Перетвори блок кейсу на структурований кейс підтримки.
Поверни ТІЛЬКИ JSON з ключами:
- keep: boolean (true лише якщо це справжній кейс підтримки з рішенням)
...
- status="open" якщо питання є, але людина ВЗАГАЛІ не відповіла (кейс ще не завершено).
