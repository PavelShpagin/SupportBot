...
Детальніше: https://supportbot.info/ua
...
Learn more: https://supportbot.info/en
...
3. Натисніть «Перенести історію повідомлень» (Transfer message history)
...
2. Scan the QR code
3. Click "Transfer message history"
...
Більше інформації: https://supportbot.info/ua"""
...
More info: https://supportbot.info/en"""
...
class SignalCliAdapter:
    ...
    def send_group_text(
        ...
        *,
        ...
        # signal-cli prints the sent message timestamp to stdout (13-digit ms epoch).
        # We parse it so callers can track bot-reply-to-user-message mappings.
        import re as _re
        if proc.stdout:
            m = _re.search(r'\b(\d{13,})\b', proc.stdout)
            if m:
...
def _parse_group_message(obj: dict) -> Optional[InboundGroupMessage]:
    ...
    # _parse_reaction handles them instead.
    if dm.get("reaction"):
        return None
